...
function processCredentials(credentials) {
  ...
  for (let i = 0; i < credentials.publicKey.allowCredentials.length; i++) {
    credentials.publicKey.allowCredentials[i].id = str2ab(
      credentials.publicKey.allowCredentials[i].id
    );
  }
}
...
async function get_credentials() {
  let cred = JSON.parse(
    document.getElementById("django_u2f_request").innerHTML
  );
  processCredentials(cred);
  const resp = await navigator.credentials.get(cred).catch(function () {
...
async function do_registration() {
  let opt = JSON.parse(
    document.getElementById("django_u2f_registration").innerHTML
  );
  ...
  try {
    ...
    if (
      error.message.indexOf("attempt was made to use an object that is not") >=
      0
    ) {
      handleStatus(
        "Registration Failed: Key may have already been registered."
      );
    }
    return;
  }
