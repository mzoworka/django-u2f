...
class U2FLoginView(LoginView):
    ...
    def requires_two_factor(self, user):
        return user.u2f_keys.exists() or user.backup_codes.exists() or user.totp_devices.exists()
...
class VerifySecondFactorView(OriginMixin, TemplateView):
    ...
    def get_forms(self):
        ...
        if self.request.method == "GET":
            ...
            forms = {key: form(**kwargs) for key, form in self.form_classes.items() if key != method}
